...
class SafetyScamDetector:
    """Detect financial exploitation, unsafe meetups, and manipulation."""
    ...
    def analyze(self, messages: list) -> SafetyAnalysisOutput:
        ...
        abort_flag = self._should_abort_immediately(flags)
            ...
            for i, msg in enumerate(messages):
                ...
                for pattern in meetup_patterns:
                    if re.search(pattern, text_lower):
                        private = bool(
                            re.search(
                                r"\b(my place|my apartment|come over|come to my place|come through)\b",
                                text_lower,
                            )
                        )

                        has_gaslighting = any(
                            re.search(p, text_lower) for p in gaslighting_patterns
                        )
            ...
            if intimacy_count > 0:
                flags.append(
                    SafetyFlag(
                        ...
                        evidence=(
                            f"Love bombing: {intimacy_count} intimate declarations "
                            f"in {round(days, 1)} days"
                        ),
                    )
                )
    ...
    def _generate_actions(self, flags: list, category: RiskCategory) -> list:
        ...
        if category == RiskCategory.ABORT_IMMEDIATELY:
            ...
            actions.append("STOP communication - serious red flags detected")
            ...
            actions.append("Proceed with extreme caution")
        ...
        if SafetyFlagType.MONEY_REQUEST in flag_types:
            actions.append("Money requests from dating matches are almost always scams")

        if SafetyFlagType.EARLY_PRIVATE_MEETING in flag_types:
            actions.append("Insist on meeting in public first - this is non-negotiable")
