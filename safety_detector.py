...
r”\b(special|amazing|beautiful|wonderful|perfect)\b.{0,30}\b(you|person|woman|man)\b”,
...
r”\b(my\s*(uncle|cousin|friend|mentor)).{0,30}\b(invest|trading|profit|returns)\b”,
...
ChainStep(step_id=“FINANCIAL_ASK”, signal_id=“FINANCIAL_REQUEST_DIRECT”, required=True, patterns=[
r”\b(deposit|invest|start\s*with)\b.{0,25}\b\d+\b”,
r”\b(deposit|invest|send|transfer)\b.{0,30}\b($[\d,]+|\d+\s*dollar|bitcoin|crypto)\b”,
r”\b(send|transfer|wire)\b.{0,20}\b(cashapp|zelle|venmo|paypal|bitcoin|crypto|wallet)\b”,
...
ChainStep(step_id=“VIDEO_AVOIDANCE”, signal_id=“MEETING_AVOIDANCE”, required=False, patterns=[
...
ChainStep(step_id=“ISOLATION”, signal_id=“COERCION_ISOLATION”, required=False, patterns=[
...
r”\b(move|switch|talk|chat|message)\b.{0,20}\b(off|outside|away\s*from|another\s*app)\b”,
r”\b(private\s*number|personal\s*phone|my\s*direct)\b”,
]),
ChainStep(step_id=“FINANCIAL_REQUEST”, signal_id=“FINANCIAL_REQUEST_DIRECT”, patterns=[
r”\b(send|transfer|wire|zelle|cashapp|venmo|paypal)\b.{0,30}\b($|\d+\s*dollar|money)\b”,
r”\b(gift\s*card|itunes|google\s*play|amazon\s*card)\b”,
r”\b(western\s*union|moneygram|bitcoin|crypto)\b.{0,20}\b(send|transfer|pay)\b”,
]),
],
),
ChainDefinition(
...
ChainStep(step_id=“GUILT_INVERSION”, signal_id=“COERCION_GUILT_INVERSION”, patterns=[
r”\b(if\s*you\s*(love|care|trust))\b.{0,20}\b(you\s*would|you.d)\b”,
...
r”\b(expose|ruin|destroy|hurt)\b.{0,20}\b(you|your\s*(family|life|reputation))\b”,
...
r”\b(calling\s*from|representative\s*of|on\s*behalf\s*of)\b.{0,20}\b(bank|irs|federal|agency)\b”,
...
r”\b(or\s*(else|you\s*will)|if\s*you\s*don.t)\b.{0,20}\b(arrest|charge|penalty|fine)\b”,
r”\b(suspend|freeze|close)\b.{0,20}\b(account|card|access)\b”,
]),
ChainStep(step_id=“PAYMENT_DEMAND”, signal_id=“FINANCIAL_REQUEST_DIRECT”, patterns=[
r”\b(pay|send|wire|transfer)\b.{0,20}\b($[\d,]+|\d+\s*dollar|fine|fee|bail)\b”,
r”\b(gift\s*card|itunes|bitcoin|crypto|wire\s*transfer)\b.{0,20}\b(pay|settle|clear)\b”,
]),
],
),
]
...
def _scan_chain(self, chain_def: ChainDefinition, turn_history: List[Dict]) -> Optional[ChainMatch]:
    steps_matched: List[Dict] = []
    last_matched_idx = -1
    ...
    optional_bonus = (optional_matched / optional_total * 0.08) if optional_total > 0 else 0.0

    completion_ratio = required_matched / required_total
    confidence = min(round(chain_def.base_confidence * completion_ratio + optional_bonus, 3), 0.99)
...
HIGH_SIGNALS     = {k for k, v in SIGNAL_WEIGHTS.items() if 0.70 <= v < 0.85}
MEDIUM_SIGNALS   = {k for k, v in SIGNAL_WEIGHTS.items() if v < 0.70}

PATTERN_LIBRARY = {
“FINANCIAL_REQUEST_DIRECT”: [
r”\b(send|transfer|wire)\b.{0,40}\b(zelle|venmo|cashapp|cash\s*app|paypal|western\s*union|moneygram|bitcoin|crypto|gift\s*card)\b”,
r”\b(zelle|venmo|cashapp|cash\s*app|paypal|western\s*union|moneygram)\b.{0,40}\b($[\d,]+|\d+\s*dollar|\d+\s*usd)\b”,
...
“GROOMING_PERSONA”: [
r”\b(widower|widow|single\s+father|single\s+mother|divorced).{0,30}\b(child|daughter|son)\b”,
...
“body”: “This conversation contains financial fraud signals. Do not send any money, gift cards, cryptocurrency, or financial account information to this person.”},
{“priority”: “high”, “title”: “Cease Contact Immediately”,
“body”: “Disengage from this conversation. Block the sender on this platform and any other platform they have asked you to use.”},
{“priority”: “high”, “title”: “Report This Account”,
“body”: “Use the platform’s report function to flag this account. Consider reporting to the FTC at reportfraud.ftc.gov or IC3.gov.”},
],
“WARN”: [
{“priority”: “high”, “title”: “Caution — Suspicious Patterns Detected”,
“body”: “This conversation contains behavioral patterns consistent with known scam or manipulation tactics. Proceed with caution.”},
{“priority”: “medium”, “title”: “Verify Identity Before Proceeding”,
“body”: “Request a live video call before sharing personal information or making any financial decisions. Legitimate contacts will not refuse this.”},
{“priority”: “medium”, “title”: “Do Not Share Financial Information”,
“body”: “Do not provide bank account details, credit card numbers, social security numbers, or send money until identity is fully verified.”},
],
“ALLOW”: [
{“priority”: “low”, “title”: “No Immediate Safety Concerns Detected”,
“body”: “This conversation does not contain known high-risk behavioral signals at this time. Continue standard safety practices.”},
{“priority”: “low”, “title”: “Continue Standard Dating Safety Practices”,
“body”: “Always meet in public places, inform a friend of your plans, and never send money to someone you have not met in person.”},
],
}
    ...
    recs = [
        {
            ...
            "body": f"This conversation matches a known {top_chain.scam_category} scam sequence spanning {top_chain.span_turns} messages. This is not a single suspicious message — it is a behavioral pattern.",
        }
    ] + recs
